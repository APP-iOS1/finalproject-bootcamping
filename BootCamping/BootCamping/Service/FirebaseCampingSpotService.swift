...
struct ReadDocuments {
    ...
    //addr1
    var campingSpotLocation: [Filterings] = []
    //lctCl
    var campingSpotView: [Filterings] = []
    //facltNm
        ...
        Future<ReadDocuments, Error> { promise in
            if readDocument.lastDoc == nil {
                if readDocument.campingSpotLocation != [] {
                    readDocument.campingSpotLocation.forEach { doc in
                        database.collection("CampingSpotList")
                            .order(by: "contentId", descending: false)
                            .whereField("doNm", isEqualTo: String(doc.filterName))
                            .limit(to: 10)
                ...
                if readDocument.campingSpotView != [] {
                    readDocument.campingSpotView.forEach { doc in
                        database.collection("CampingSpotList")
                            .whereField("lctCl", isEqualTo: doc.filterName)
                            .order(by: "contentId", descending: false)
                            .limit(to: 10)
                            .getDocuments { snapshot, error in
                                if let error = error {
