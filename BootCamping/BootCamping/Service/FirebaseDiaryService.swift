...
struct FirebaseDiaryService {
    ...
    func mostLikedGetDiarysService() -> AnyPublisher<[UserInfoDiary], Error> {
        Future<[UserInfoDiary], Error> { promise in
            database.collection("Diarys").whereField("diaryIsPrivate", isEqualTo: false).whereField("diaryCreatedDate", isGreaterThan: Timestamp(date: Date(timeIntervalSinceNow: -30 * 24 * 60 * 60))).getDocuments { snapshot, error in
                if let error = error {
                    print(error)
                }
                guard let snapshot = snapshot else {
