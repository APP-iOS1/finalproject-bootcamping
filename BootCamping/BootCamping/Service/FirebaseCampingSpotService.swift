    ...
    func readCampingSpotService(readDocument: ReadDocuments) -> AnyPublisher<ReadDocuments, Error> {
        Future<ReadDocuments, Error> { promise in
            if readDocument.lastDoc == nil {
                if readDocument.campingSpotLocation != nil {
                    database.collection("CampingSpotList")
                        .whereField("addr1", arrayContains: String(readDocument.campingSpotLocation ?? ""))
                ...
                if readDocument.campingSpotView != nil {
                    database.collection("CampingSpotList")
                        .whereField("lctCl", arrayContains: readDocument.campingSpotView ?? [])
                        .order(by: "contentId", descending: false)
                        .limit(to: 10)
                        .getDocuments { snapshot, error in
                            if let error = error {
                ...
                if readDocument.campingSpotName != nil {
                    database.collection("CampingSpotList")
                ...
                if readDocument.campingSpotContenId != nil {
                    database.collection("CampingSpotList")
                        .whereField("contentId", arrayContains: String(readDocument.campingSpotContenId ?? ""))
                        .order(by: "contentId", descending: false)
                        .limit(to: 10)
                        .getDocuments { snapshot, error in
                            if let error = error {
